...
function getPhaseSystemPrompt(phase: typeof BUILD_PHASES[number], cssVars?: string): string {
  return [
    ...
    '',
    '## ABSOLUTE RULES:',
    '- You MUST call generate_website tool — NEVER respond with plain text',
    '- All content MUST be in Arabic (100%) with Saudi-style data',
    ...
    '- No external images — use gradients, SVG, patterns, CSS art',
    '- No function declarations, export, import — raw JSX only',
    '- Every element must have hover/focus/active states',
    '',
    '## PHASE INSTRUCTION:',
    `You are generating Phase ${phase.id}/4: "${phase.labelEn}"`,
    `${phase.description}`,
    '',
    `Generate EXACTLY these files: ${phase.files.join(", ")}`,
    'Do NOT generate any other files. Focus ALL your output on making these files rich and complete.',
    '',
    ...
    '',
    '## QUALITY CHECKLIST:',
    `- [ ] Generate EXACTLY ${phase.files.length} files: ${phase.files.join(", ")}`,
    '- [ ] Each .tsx component 200-400 lines of rich JSX',
    '- [ ] Every interactive element has hover + focus + active states',
    '- [ ] Complex gradients (via- keyword)',
    '- [ ] Multiple shadow layers',
    '- [ ] All text Arabic with Saudi data',
